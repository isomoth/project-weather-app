...
fetch(API_URL)
  ...
  .catch(
    (data) =>
      (text.innerHTML += `<h1>Sorry, API not working right now. Please wait a few minutes and try again!</h1>`)
  );
...
fetch(API_URL_5DAY)
  ...
  .then((data) => {
    ...
    // Loop to display the forecast on every weekday
    for (let i = 0; i < 6; i++) {
      forecastContainer.innerHTML += `
	  <div class="days">${getWeekDay(filteredForecast[i].dt)}</div>
      <div class="temp">${filteredForecast[i].main.temp.toFixed(0)}°</div>
